...
//        Collections.shuffle(imageUrlsArr);
    ...
    private String getAdText() {
        ...
        text += "активирован, для пользования в " + country + ", версия" +
                " под оператора " + GadgetConst.MAP_COUNTRIES_OPERATOR.get(country) + ", " +
                "также имеются и другие модели<br>";
        text += "✅ перед визитом в магазин, просим бронировать интересующий вас товар";
        if (metaModel.contains(Gadgets.MEMORY_GB)) {
            text += "</p><p>\uD83D\uDC9BМы всегда идем навстречу нашим покупателям.<br>" +
            ...
            text += "\uD83D\uDD1DМы занимаемся продажей смартфонов более 5 лет!</p>";
            text += "</p><p>";
            text += "✔ гарантия на обмен 14 дней при возникновении заводского брака<br>";
