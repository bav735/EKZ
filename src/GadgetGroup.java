    ...
    private String getAdText() {
        ...
        if (parent.companyName.equals(Gadgets.AMOLED)) {
            ...
            if (getVendor().equals("Apple")) {
                ...
                if (isOnlyNew) {
                    text += "- Телефоны абсолютно новый.<br>";
                } else {
                    text += "- В нашем ассортименте имеются новые и официально " +
                            "восстановленные телефоны данного производителя.<br>";
                ...
                text += "- В нашем ассортименте имеются только новые телефоны данного" +
                        " производителя.<br>";
            }
            text += questionNum++ + ") Телефон активированный?<br>";
            text += "- Нет, статус активации вы сможете проверить на сайте" +
                    " производителя непосредственно перед покупкой.<br>";
            ...
            text += "- Гарантия производителя сроком 1 год с момента активации" +
                    " телефона действует в " + country + ", однако в продаже" +
                    " имеются и другие модели.<br>";
            ...
            text += "- Вы можете забронировать телефон для самовывоза либо заказать " +
                    "доставку. Операторы колл-центра принимают заказы с 9:00 до " +
                    "21:00, без выходных. В остальное время вы можете оставить нам " +
                    "сообщение через интерфейс Авито или оставить заказ на" +
                    " сайте iSPARK.<br>";
            text += questionNum++ + ") Есть ли срочная доставка и как она происходит?<br>";
            text += "- Курьерская доставка день в день осуществляется по Москве и " +
                    "Казани. Стоимость доставки по Москве составляет 400 руб в " +
                    "пределах МКАД и +30 руб за каждый км после МКАД. Стоимость " +
                    "доставки по Казани составляет 300 руб. После получения и проверки " +
                    "телефона вы оплачиваете курьеру стоимость телефона и доставки.<br>";
            text += questionNum++ + ") Нужна ли предоплата при отправке телефона в " +
                    "другой город?<br>";
            text += "- Необходима частичная предоплата заказа в размере стоимости " +
                    "доставки телефона " +
                    "в ваш город (составляет не более 600 руб). Оставшуюся сумму" +
            ...
            text += "- К купленному телефону идет в подарок портативный " +
                    "PowerBank за любой (негативный или позитивный) отзыв о нас." +
            ...
            text += "- Оплата принимается от юридических лиц, работающих как" +
                    " с НДС, так и без НДС. При этом удерживается комиссия 5%.<br>";
            text += questionNum++ + ") Можно ли сдать свой телефон на выкуп или " +
                    "обменять на новый и как это сделать?<br>";
            text += "- Обмен либо выкуп действует только для телефонов производителей" +
                    ", представленных в " +
                    "нашем ассортименте. Обмен производится с доплатой в любую из сторон. " +
            ...
            text += questionNum++ + ") Есть покупка нескольких телефонов оптом?<br>";
